#!/usr/bin/env python3
"""
Process local MIMIC-IV data into parquet format for training.

This script reads MIMIC-IV CSV files from MIMIC_DATA/MIMIC-IV/ directory
and creates a processed parquet file ready for model training.

Usage:
    python scripts/process_mimic4_local.py --output data/processed/mimic4_full.parquet
    
    # Process with limit for testing
    python scripts/process_mimic4_local.py --output data/processed/mimic4_test.parquet --limit 1000
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Process local MIMIC-IV data into parquet format"
    )
    
    parser.add_argument(
        "--notes-csv",
        type=str,
        default="MIMIC_DATA/MIMIC-IV/discharge.csv.gz",
        help="Path to discharge.csv.gz (default: MIMIC_DATA/MIMIC-IV/discharge.csv.gz)"
    )
    parser.add_argument(
        "--diagnoses-csv",
        type=str,
        default="MIMIC_DATA/MIMIC-IV/diagnoses_icd.csv.gz",
        help="Path to diagnoses_icd.csv.gz (default: MIMIC_DATA/MIMIC-IV/diagnoses_icd.csv.gz)"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="data/processed/mimic4_full.parquet",
    ...
    parser.add_argument(
        ...
        type=int,
        choices=[9, 10],
        help="Filter by ICD version (9 or 10). If not specified, include both."
    )
    ...
    if not notes_path.exists():
        ...
        logger.error("Please ensure MIMIC-IV data is in MIMIC_DATA/MIMIC-IV/")
        sys.exit(1)
    ...
    logger.info("=" * 70)
    logger.info("MIMIC-IV Data Processing")
    logger.info("=" * 70)
    ...
    if args.icd_version:
        logger.info(f"ICD version filter: {args.icd_version}")
    logger.info("=" * 70)
    ...
    logger.info(f"   python scripts/evaluate.py --model caml --checkpoint checkpoints/caml_mimic3/best_model.pt --data {output_path}")
